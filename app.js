...
const CustomerFollowUpApp = () => {
  ...
  useEffect(() => {
    ...
    checkNotifications(savedCustomers);
  }, []);

  useEffect(() => {
    localStorage.setItem('customers', JSON.stringify(customers));
    checkNotifications(customers);
  }, [customers]);

  const checkNotifications = (customerList) => {
      ...
      customer.products?.forEach(product => {
        ...
        if (daysSinceLastFollowUp >= 14) {
          pending.push({
            ...
            daysDue: daysSinceLastFollowUp - 14
          });
        }
      });
    });
        ...
        return {
          ...
          products: customer.products.map(product => {
            if (product.id === productId) {
              return {
                ...product,
                followUps: [followUpData, ...(product.followUps || [])]
              };
            }
            return product;
          })
    ...
    <div className="h-screen bg-gray-50 flex flex-col">
      <div className="bg-blue-600 text-white p-4 shadow-lg">
        <div className="flex items-center justify-between mb-3">
          <h1 className="text-xl font-bold">Kunden Follow-up</h1>
          <button
            ...
            {filteredCustomers.map(customer => {
              ...
              return (
                <div
                  ...
                  className="p-4 hover:bg-gray-100 cursor-pointer active:bg-gray-200"
                >
      ...
      {showAddProduct && selectedCustomer && (
        <AddProductModal
          ...
          onSave={(data) => addProductToCustomer(selectedCustomer.id, data)}
        />
      )}
      ...
      {showAddFollowUp && selectedCustomer && (
        <AddFollowUpModal
          customer={selectedCustomer}
          productId={showAddFollowUp}
          onClose={() => setShowAddFollowUp(false)}
          onSave={(data) => addFollowUp(selectedCustomer.id, showAddFollowUp, data)}
        />
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
      <div className="absolute inset-0 overflow-y-auto">
        <div className="min-h-full bg-white">
          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-lg">Produkte</h3>
              <button
            ...
            {(!customer.products || customer.products.length === 0) ? (
              <p className="text-gray-500 text-center py-8">Noch keine Produkte hinzugefügt</p>
            ) : (
      ...
      <div className="bg-white rounded-lg w-full max-w-md">
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold mb-1">Gesundheitliche Herausforderungen</label>
            <textarea
              value={formData.healthChallenges}
              onChange={(e) => setFormData({...formData, healthChallenges: e.target.value})}
              className="w-full border rounded-lg px-3 py-2 h-24"
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-md">
        ...
        <div className="p-4 space-y-4">
          <div>
            <label className="block text-sm font-semibold mb-1">Produktname *</label>
            <input
              type="text"
              value={formData.name}
              onChange={(e) => setFormData({...formData, name: e.target.value})}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border rounded-lg px-3 py-2 h-24"
              placeholder="Anfangsbeobachtungen, Dosierung..."
            />
          </div>
          <div className="flex gap-2">
            <button
...
const AddFollowUpModal = ({ customer, productId, onClose, onSave }) => {
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold mb-1">
              Was hat sich verbessert? Erfahrungen? *
            </label>
            <textarea
              value={formData.notes}
              onChange={(e) => setFormData({...formData, notes: e.target.value})}
